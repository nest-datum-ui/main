...
const onCreate = async ({
	...
	try {
		...
		const { fieldId } = Store().getState()['api'].form[entityId];
		const errors = {};

		apiPath = `field/${fieldId}/options/${entityId}`;
		...
		actionApiFormCreate({
			...
			path: `field/${fieldId}/options/${entityId}`,
			...
			await actionDialogClose('formsFieldReferenceItem')();
