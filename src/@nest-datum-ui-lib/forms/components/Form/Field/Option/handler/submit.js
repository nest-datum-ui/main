...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FORMS_PATH_FIELD_OPTION, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FORMS_PATH_FIELD_OPTION, 'errors', errors)();
		await actionApiFormProp(FORMS_PATH_FIELD_OPTION, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FORMS_PATH_FIELD_OPTION, entityId)();
	}
	else {
		actionApiFormCreate(FORMS_PATH_FIELD_OPTION)();
	}
};
