...
import SelectFieldStatus from '@nest-datum-ui-lib/forms/components/Select/Field/Status';
...
let Field = () => {
	...
	const fieldStatusId = useSelector(selectorMainExtract([ 'api', 'form', entityId, 'fieldStatusId' ]));
	...
	const errorFieldStatusId = useSelector(selectorMainExtract([ 'api', 'form', entityId, 'errors', 'fieldStatusId' ]));
	...
	const onChangeFieldStatusId = React.useCallback((e, newValue) => {
		actionApiFormProp(entityId, 'fieldStatusId', e.target.value)();
	}, [
		entityId,
	]);
		...
		<form 
			...
			<Box py={2}>
				<SelectFieldStatus
					...
					name="fieldStatusId"
					value={fieldStatusId || ''}
					onChange={onChangeFieldStatusId}
					error={errorFieldStatusId} />
			</Box>
			<Box py={2}>
				<InputBool
					disabled={loader}
