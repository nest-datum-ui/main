...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FORMS_PATH_FIELD_STATUS, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FORMS_PATH_FIELD_STATUS, 'errors', errors)();
		await actionApiFormProp(FORMS_PATH_FIELD_STATUS, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FORMS_PATH_FIELD_STATUS, entityId)();
	}
	else {
		actionApiFormCreate(FORMS_PATH_FIELD_STATUS)();
	}
};
