...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FORMS_PATH_SETTING, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FORMS_PATH_SETTING, 'errors', errors)();
		await actionApiFormProp(FORMS_PATH_SETTING, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FORMS_PATH_SETTING, entityId)();
	}
	else {
		actionApiFormCreate(FORMS_PATH_SETTING)();
	}
};
