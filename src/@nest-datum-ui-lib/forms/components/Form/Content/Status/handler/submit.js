...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FORMS_PATH_CONTENT_STATUS, 'loader', true)();
	...
	const {
		...
		.getState()
		.api || {})
		.form || {})[FORMS_PATH_CONTENT_STATUS] || {};
	const errors = {};
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FORMS_PATH_CONTENT_STATUS, 'errors', errors)();
		await actionApiFormProp(FORMS_PATH_CONTENT_STATUS, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FORMS_PATH_CONTENT_STATUS, entityId)();
	}
	else {
		actionApiFormCreate(FORMS_PATH_CONTENT_STATUS)();
	}
};
