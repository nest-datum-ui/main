...
const onCreate = async ({
	...
	try {
		...
		const { formId } = Store().getState()['api'].form[entityId];
		const errors = {};

		apiPath = `form/${formId}/options/${entityId}`;
		...
		actionApiFormCreate({
			...
			path: `form/${formId}/options/${entityId}`,
			...
			await actionDialogClose('formsFormReferenceItem')();
