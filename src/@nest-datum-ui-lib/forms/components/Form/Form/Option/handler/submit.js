...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FORMS_PATH_FORM_OPTION, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FORMS_PATH_FORM_OPTION, 'errors', errors)();
		await actionApiFormProp(FORMS_PATH_FORM_OPTION, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FORMS_PATH_FORM_OPTION, entityId)();
	}
	else {
		actionApiFormCreate(FORMS_PATH_FORM_OPTION)();
	}
};
