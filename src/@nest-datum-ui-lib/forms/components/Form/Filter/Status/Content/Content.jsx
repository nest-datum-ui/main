...
import FormsSelectContentStatusMultiple from '@nest-datum-ui-lib/forms/components/Select/Content/Status/Multiple';
...
let Content = ({
	...
	const dataLength = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_CONTENT_STATUS, 'data', 'length' ]));
	const value = (dataLength > 0)
		? utilsUrlItemFilterGetId(search, FORMS_PATH_CONTENT_STATUS, 'contentStatusId')
		: [];
	const onChangeMemo = React.useCallback((e) => {
		actionApiListCreateFilterUrl(e, utilsUrlItemFilterId('contentStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
	...
	const onClear = React.useCallback((e) => {
		actionApiListClearFilterUrl(utilsUrlItemFilterId('contentStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
	...
	return <React.Fragment>
		<Grid
			...
			<Grid
				item
				xs={true}>
				<FormsSelectContentStatusMultiple 
					label="Status"
					name={id}
