...
import FormsSelectFieldStatusMultiple from '@nest-datum-ui-lib/forms/components/Select/Field/Status/Multiple';
...
let Field = ({
	...
	const dataLength = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_STATUS, 'data', 'length' ]));
	const value = (dataLength > 0)
		? utilsUrlItemFilterGetId(search, FORMS_PATH_FIELD_STATUS, 'fieldStatusId')
		: [];
	const onChangeMemo = React.useCallback((e) => {
		actionApiListCreateFilterUrl(e, utilsUrlItemFilterId('fieldStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
	...
	const onClear = React.useCallback((e) => {
		actionApiListClearFilterUrl(utilsUrlItemFilterId('fieldStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
	...
	return <React.Fragment>
		<Grid
			...
			<Grid
				item
				xs={true}>
				<FormsSelectFieldStatusMultiple 
					label="Status"
					name={id}
