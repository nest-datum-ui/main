...
let Drop = () => {
	const entityId = useSelector(selectorMainExtract([ 'dialog', FORMS_PATH_FORM_OPTION_RELATION, 'entityId' ]));
	const formLoader = useSelector(selectorMainExtract([ 'api', 'form', FORMS_PATH_FORM_OPTION_RELATION, 'loader' ]));
	const listLoader = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FORM_OPTION_RELATION, 'loader' ]));
	const onDrop = React.useCallback((e) => actionApiFormDrop(FORMS_PATH_FORM_OPTION_RELATION, entityId, { sliceInList: true })(), [
		entityId,
	]);
	...
	return <React.Fragment>
		<DialogOptionDrop 
			...
			maxWidth="xs"
			id={FORMS_PATH_FORM_OPTION_RELATION}
			onHandle={onDrop} />
	</React.Fragment>;
};
