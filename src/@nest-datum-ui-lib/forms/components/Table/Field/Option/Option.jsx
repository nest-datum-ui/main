...
import { 
	...
	FORMS_PATH_FIELD_OPTION_CREATE, 
...
let Option = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_OPTION, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_OPTION, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_OPTION, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_OPTION, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_OPTION, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', FORMS_PATH_FIELD_OPTION ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(FORMS_PATH_FIELD_OPTION, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(FORMS_PATH_FIELD_OPTION, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(FORMS_PATH_FIELD_OPTION, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(FORMS_PATH_FIELD_OPTION, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(FORMS_PATH_FIELD_OPTION, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(FORMS_PATH_FIELD_OPTION, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(FORMS_PATH_FIELD_OPTION, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(FORMS_PATH_FIELD_OPTION, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(FORMS_PATH_FIELD_OPTION), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(FORMS_PATH_FIELD_OPTION, 'loader', true)(), [
	]);
	...
	React.useEffect(() => () => actionApiListClear(FORMS_PATH_FIELD_OPTION)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={FORMS_PATH_FIELD_OPTION_CREATE} />}
			storePath={storePath}
		...
		<TableOption
			...
			{utilsCheckArr(data)
				&& data.map((item) => <TableOptionItem
					...
					key={item.id}
					id={item.id}
					to={`/forms/field/options/${item.id}`}
					name={item.name}
