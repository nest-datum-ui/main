...
import { 
	FORMS_PATH_FIELD_STATUS,
	FORMS_PATH_FIELD_STATUS_CREATE, 
...
let Status = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_STATUS, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_STATUS, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_STATUS, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_STATUS, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', FORMS_PATH_FIELD_STATUS, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', FORMS_PATH_FIELD_STATUS ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(FORMS_PATH_FIELD_STATUS, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(FORMS_PATH_FIELD_STATUS, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(FORMS_PATH_FIELD_STATUS, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(FORMS_PATH_FIELD_STATUS, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(FORMS_PATH_FIELD_STATUS, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(FORMS_PATH_FIELD_STATUS, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(FORMS_PATH_FIELD_STATUS, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(FORMS_PATH_FIELD_STATUS, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(FORMS_PATH_FIELD_STATUS), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(FORMS_PATH_FIELD_STATUS, 'loader', true)(), [
	]);
	...
	React.useEffect(() => {
		if (!unmount) {
			actionApiListGet(FORMS_PATH_FIELD_STATUS, {
				page,
				limit,
				query,
				select,
	...
	React.useEffect(() => () => actionApiListClear(FORMS_PATH_FIELD_STATUS)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={FORMS_PATH_FIELD_STATUS_CREATE} />}
			storePath={storePath}
		...
		<TableStatus
			...
			{utilsCheckArr(data)
				&& data.map((item) => <TableStatusItem
					...
					key={item.id}
					id={item.id}
					to={`/forms/field/statuses/${item.id}`}
					name={item.name}
