...
let Options = ({
	...
	const onDelete = React.useCallback((fieldId) => (e) => {
		actionDialogOpen('formsFieldReferenceDrop', {
			fieldId,
			optionId: entityId,
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										<IconButton onClick={onDelete(item['fieldFieldOptions'][0]['id'])}>
...
Options.defaultProps = {
	...
	storeName: 'formsFieldOptionList',
