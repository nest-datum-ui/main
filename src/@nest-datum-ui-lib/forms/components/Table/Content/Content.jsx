...
import MenuContentContext from '@nest-datum-ui-lib/forms/components/Menu/Content/Context';
...
let Content = ({
	...
	const onMenu = React.useCallback((itemId) => (e) => {
		actionMenuOpen(`forms-menu-content-context-${itemId}`, e.target)();
	}, [
	]);
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										...
										<MenuContentContext 
											id={`forms-menu-content-context-${item.id}`}
											entityId={item.id}
