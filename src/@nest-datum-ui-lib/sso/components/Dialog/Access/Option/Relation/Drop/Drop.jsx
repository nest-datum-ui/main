...
let Drop = () => {
	const entityId = useSelector(selectorMainExtract([ 'dialog', SSO_PATH_ACCESS_OPTION_RELATION, 'entityId' ]));
	const formLoader = useSelector(selectorMainExtract([ 'api', 'form', SSO_PATH_ACCESS_OPTION_RELATION, 'loader' ]));
	const listLoader = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ACCESS_OPTION_RELATION, 'loader' ]));
	const onDrop = React.useCallback((e) => actionApiFormDrop(SSO_PATH_ACCESS_OPTION_RELATION, entityId, { sliceInList: true })(), [
		entityId,
	]);
	...
	return <React.Fragment>
		<DialogOptionDrop 
			...
			maxWidth="xs"
			id={SSO_PATH_ACCESS_OPTION_RELATION}
			onHandle={onDrop} />
	</React.Fragment>;
};
