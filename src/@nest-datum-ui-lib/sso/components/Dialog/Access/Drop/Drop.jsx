...
let Drop = () => {
	const entityId = useSelector(selectorMainExtract([ 'dialog', SSO_PATH_ACCESS, 'entityId' ]));
	const formLoader = useSelector(selectorMainExtract([ 'api', 'form', SSO_PATH_ACCESS, 'loader' ]));
	const listLoader = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ACCESS, 'loader' ]));
	const onDrop = React.useCallback((e) => actionApiFormDrop(SSO_PATH_ACCESS, entityId)(), [
		entityId,
	]);
	...
	return <React.Fragment>
		<Dialog 
			...
			<DialogContentText>
				Are you sure you want to delete the current access? This operation is irreversible and may compromise data integrity.
			</DialogContentText>
		</Dialog>
	</React.Fragment>;
};
