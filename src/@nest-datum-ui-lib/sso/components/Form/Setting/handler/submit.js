...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(SSO_PATH_SETTING, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(SSO_PATH_SETTING, 'errors', errors)();
		await actionApiFormProp(SSO_PATH_SETTING, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(SSO_PATH_SETTING, entityId)();
	}
	else {
		actionApiFormCreate(SSO_PATH_SETTING)();
	}
};
