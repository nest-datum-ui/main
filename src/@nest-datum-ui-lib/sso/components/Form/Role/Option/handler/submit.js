...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(SSO_PATH_ROLE_OPTION, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(SSO_PATH_ROLE_OPTION, 'errors', errors)();
		await actionApiFormProp(SSO_PATH_ROLE_OPTION, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(SSO_PATH_ROLE_OPTION, entityId)();
	}
	else {
		actionApiFormCreate(SSO_PATH_ROLE_OPTION)();
	}
};
