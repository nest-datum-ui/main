...
const onCreate = async ({
	...
	try {
		...
		const { roleId } = Store().getState()['api'].form[entityId];
		const errors = {};

		apiPath = `role/${roleId}/options/${entityId}`;

		if (!validateStr(roleId, true)) {
			errors['roleId'] = 'The value is in the wrong format.';
		}
		else if (!validateNotEmpty(roleId)) {
			errors['roleId'] = 'Cannot be empty.';
		}
		...
		actionApiFormCreate({
			...
			path: `role/${roleId}/options/${entityId}`,
			...
			await actionDialogClose('ssoRoleReferenceItem')();
