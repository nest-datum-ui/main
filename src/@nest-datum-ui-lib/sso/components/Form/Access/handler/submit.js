...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(SSO_PATH_ACCESS, 'loader', true)();
	...
	!utilsCheckStrId(accessStatusId)
		&& (errors['accessStatusId'] = 'The value is in the wrong format.');
	(utilsCheckExists(isNotDelete) && !utilsCheckBool(isNotDelete))
		&& (errors['isNotDelete'] = 'The value is in the wrong format.');

	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(SSO_PATH_ACCESS, 'errors', errors)();
		await actionApiFormProp(SSO_PATH_ACCESS, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(SSO_PATH_ACCESS, entityId)();
		actionApiFormCreateOptions(SSO_PATH_ACCESS_OPTION, { path: `${SSO_PATH_ACCESS}/${entityId}/options` })();
	}
	else {
		actionApiFormCreate(SSO_PATH_ACCESS)();
		actionApiFormCreateOptions(SSO_PATH_ACCESS_OPTION, { path: `${SSO_PATH_ACCESS}/${entityId}/options` })();
	}
};
