...
import { SSO_PATH_ACCESS_OPTION } from '@nest-datum-ui-lib/sso/consts/path.js';
...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(SSO_PATH_ACCESS_OPTION, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(SSO_PATH_ACCESS_OPTION, 'errors', errors)();
		await actionApiFormProp(SSO_PATH_ACCESS_OPTION, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(SSO_PATH_ACCESS_OPTION, entityId)();
	}
	else {
		actionApiFormCreate(SSO_PATH_ACCESS_OPTION)();
	}
};
