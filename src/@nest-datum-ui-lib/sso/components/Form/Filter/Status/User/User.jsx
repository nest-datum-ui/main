...
import SsoSelectUserStatusMultiple from '@nest-datum-ui-lib/sso/components/Select/User/Status/Multiple';
...
let User = ({
	...
	const dataLength = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_USER_STATUS, 'data', 'length' ]));
	const value = (dataLength > 0)
		? utilsUrlItemFilterGetId(search, SSO_PATH_USER_STATUS, 'userStatusId')
		: [];
	const onChangeMemo = React.useCallback((e) => {
		actionApiListCreateFilterUrl(e, utilsUrlItemFilterId('userStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
	...
	const onClear = React.useCallback((e) => {
		actionApiListClearFilterUrl(utilsUrlItemFilterId('userStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
