...
import SsoSelectRoleStatusMultiple from '@nest-datum-ui-lib/sso/components/Select/Role/Status/Multiple';
...
let Role = ({
	...
	const dataLength = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_STATUS, 'data', 'length' ]));
	const value = (dataLength > 0)
		? utilsUrlItemFilterGetId(search, SSO_PATH_ROLE_STATUS, 'roleStatusId')
		: [];
	const onChangeMemo = React.useCallback((e) => {
		actionApiListCreateFilterUrl(e, utilsUrlItemFilterId('roleStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
	...
	const onClear = React.useCallback((e) => {
		actionApiListClearFilterUrl(utilsUrlItemFilterId('roleStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
