...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(SSO_PATH_USER_STATUS, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(SSO_PATH_USER_STATUS, 'errors', errors)();
		await actionApiFormProp(SSO_PATH_USER_STATUS, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(SSO_PATH_USER_STATUS, entityId)();
	}
	else {
		actionApiFormCreate(SSO_PATH_USER_STATUS)();
	}
};
