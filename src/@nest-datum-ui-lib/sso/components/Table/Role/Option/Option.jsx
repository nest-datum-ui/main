...
let Option = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_OPTION, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_OPTION, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_OPTION, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_OPTION, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_OPTION, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', SSO_PATH_ROLE_OPTION ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(SSO_PATH_ROLE_OPTION, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(SSO_PATH_ROLE_OPTION, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(SSO_PATH_ROLE_OPTION, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(SSO_PATH_ROLE_OPTION, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(SSO_PATH_ROLE_OPTION, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(SSO_PATH_ROLE_OPTION, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(SSO_PATH_ROLE_OPTION, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(SSO_PATH_ROLE_OPTION, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(SSO_PATH_ROLE_OPTION), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(SSO_PATH_ROLE_OPTION, 'loader', true)(), [
	]);
	...
	React.useEffect(() => () => actionApiListClear(SSO_PATH_ROLE_OPTION)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={SSO_PATH_ROLE_OPTION_CREATE} />}
			storePath={storePath}
