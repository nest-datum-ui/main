...
import SsoFormFilterStatusRole from '@nest-datum-ui-lib/sso/components/Form/Filter/Status/Role';
...
let Role = () => {
	...
	const storePath = React.useMemo(() => [ 'api', 'list', SSO_PATH_ROLE ], [
	]);
	...
	const onSortId = React.useCallback((value) => actionApiListSort(SSO_PATH_ROLE, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(SSO_PATH_ROLE, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(SSO_PATH_ROLE, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(SSO_PATH_ROLE, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(SSO_PATH_ROLE, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(SSO_PATH_ROLE, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(SSO_PATH_ROLE), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(SSO_PATH_ROLE, 'loader', true)(), [
	]);
	...
	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={SSO_PATH_ROLE_CREATE} />}
			storePath={storePath}
			...
			<SsoFormFilterStatusRole onInput={onLoader} />
		</FormFilter>
		{(!displayLoader)
			&& <React.Fragment>
				<TablePagination
