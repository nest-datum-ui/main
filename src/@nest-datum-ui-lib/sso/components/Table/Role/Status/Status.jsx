...
import { 
	SSO_PATH_ROLE_STATUS,
	SSO_PATH_ROLE_STATUS_CREATE, 
...
let Status = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_STATUS, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_STATUS, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_STATUS, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_STATUS, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ROLE_STATUS, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', SSO_PATH_ROLE_STATUS ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(SSO_PATH_ROLE_STATUS, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(SSO_PATH_ROLE_STATUS, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(SSO_PATH_ROLE_STATUS, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(SSO_PATH_ROLE_STATUS, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(SSO_PATH_ROLE_STATUS, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(SSO_PATH_ROLE_STATUS, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(SSO_PATH_ROLE_STATUS, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(SSO_PATH_ROLE_STATUS, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(SSO_PATH_ROLE_STATUS), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(SSO_PATH_ROLE_STATUS, 'loader', true)(), [
	]);
	...
	React.useEffect(() => {
		if (!unmount) {
			actionApiListGet(SSO_PATH_ROLE_STATUS, {
				page,
				limit,
				query,
				select,
	...
	React.useEffect(() => () => actionApiListClear(SSO_PATH_ROLE_STATUS)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={SSO_PATH_ROLE_STATUS_CREATE} />}
			storePath={storePath}
