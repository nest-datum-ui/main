...
let Options = ({
	...
	const onDelete = React.useCallback((accessId) => (e) => {
		actionDialogOpen('ssoAccessReferenceDrop', {
			accessId,
			optionId: entityId,
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										<IconButton onClick={onDelete(item['accessAccessOptions'][0]['id'])}>
