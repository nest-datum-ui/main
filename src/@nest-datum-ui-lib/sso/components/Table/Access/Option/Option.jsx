...
let Option = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ACCESS_OPTION, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ACCESS_OPTION, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ACCESS_OPTION, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ACCESS_OPTION, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', SSO_PATH_ACCESS_OPTION, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', SSO_PATH_ACCESS_OPTION ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(SSO_PATH_ACCESS_OPTION, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(SSO_PATH_ACCESS_OPTION, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(SSO_PATH_ACCESS_OPTION, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(SSO_PATH_ACCESS_OPTION, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(SSO_PATH_ACCESS_OPTION, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(SSO_PATH_ACCESS_OPTION, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(SSO_PATH_ACCESS_OPTION, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(SSO_PATH_ACCESS_OPTION, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(SSO_PATH_ACCESS_OPTION), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(SSO_PATH_ACCESS_OPTION, 'loader', true)(), [
	]);
	...
	React.useEffect(() => {
		if (!unmount) {
			actionApiListGet(SSO_PATH_ACCESS_OPTION, {
				page,
				limit,
				query,
				select,
	...
	React.useEffect(() => () => actionApiListClear(SSO_PATH_ACCESS_OPTION)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={SSO_PATH_ACCESS_OPTION_CREATE} />}
			storePath={storePath}
