...
import { 
	SSO_PATH_ACCESS_STATUS,
	SSO_PATH_ACCESS_STATUS_CREATE, 
...
let Status = () => {
	...
	const storePath = React.useMemo(() => [ 'api', 'list', SSO_PATH_ACCESS_STATUS ], [
	]);
	...
	const onSortId = React.useCallback((value) => actionApiListSort(SSO_PATH_ACCESS_STATUS, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(SSO_PATH_ACCESS_STATUS, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(SSO_PATH_ACCESS_STATUS, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(SSO_PATH_ACCESS_STATUS, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(SSO_PATH_ACCESS_STATUS, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(SSO_PATH_ACCESS_STATUS, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(SSO_PATH_ACCESS_STATUS), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(SSO_PATH_ACCESS_STATUS, 'loader', true)(), [
	]);
	...
	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={SSO_PATH_ACCESS_STATUS_CREATE} />}
			storePath={storePath}
		...
		<TableStatus
			...
			{utilsCheckArr(data)
				&& data.map((item) => <TableStatusItem
					...
					key={item.id}
					id={item.id}
					to={`/sso/access/statuses/${item.id}`}
					name={item.name}
