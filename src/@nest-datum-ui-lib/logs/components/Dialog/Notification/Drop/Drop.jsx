...
let Drop = () => {
	const entityId = useSelector(selectorMainExtract([ 'dialog', LOGS_PATH_NOTIFICATION, 'entityId' ]));
	const formLoader = useSelector(selectorMainExtract([ 'api', 'form', LOGS_PATH_NOTIFICATION, 'loader' ]));
	const listLoader = useSelector(selectorMainExtract([ 'api', 'list', LOGS_PATH_NOTIFICATION, 'loader' ]));
	const onDrop = React.useCallback((e) => actionApiFormDrop(LOGS_PATH_NOTIFICATION, entityId)(), [
		entityId,
	]);

	return <React.Fragment>
		<Dialog 
			id={LOGS_PATH_NOTIFICATION}
