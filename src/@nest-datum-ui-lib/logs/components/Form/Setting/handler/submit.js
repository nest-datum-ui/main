...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(LOGS_PATH_SETTING, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(LOGS_PATH_SETTING, 'errors', errors)();
		await actionApiFormProp(LOGS_PATH_SETTING, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(LOGS_PATH_SETTING, entityId)();
	}
	else {
		actionApiFormCreate(LOGS_PATH_SETTING)();
	}
};
