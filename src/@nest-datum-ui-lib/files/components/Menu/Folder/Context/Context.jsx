...
let Context = ({
	...
	const onEdit = React.useCallback((e) => {
		actionDialogOpen('filesManageFolderStore', { entityId })();
		actionMenuClose()();
	}, [
		entityId,
	]);
	const onDelete = React.useCallback((e) => {
		actionDialogOpen('filesManageFolderDrop', { entityId })();
		actionMenuClose()();
	}, [
		entityId,
	]);
	const onRestore = React.useCallback(async (e) => {
		...
		await actionApiFormRestore({
			...
			const filesManageFolderList = [ ...((Store()
				...
				.data || []) ];
			const findIndex = filesManageFolderList.findIndex((file) => file.id === entityId);

			if (filesManageFolderList[findIndex]) {
				filesManageFolderList[findIndex]['isDeleted'] = false;
