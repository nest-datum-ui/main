...
import { 
	FILES_PATH_PROVIDER_STATUS,
	FILES_PATH_PROVIDER_STATUS_CREATE, 
...
let Status = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_PROVIDER_STATUS, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_PROVIDER_STATUS, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_PROVIDER_STATUS, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_PROVIDER_STATUS, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_PROVIDER_STATUS, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', FILES_PATH_PROVIDER_STATUS ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(FILES_PATH_PROVIDER_STATUS, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(FILES_PATH_PROVIDER_STATUS, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(FILES_PATH_PROVIDER_STATUS, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(FILES_PATH_PROVIDER_STATUS, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(FILES_PATH_PROVIDER_STATUS, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(FILES_PATH_PROVIDER_STATUS, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(FILES_PATH_PROVIDER_STATUS, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(FILES_PATH_PROVIDER_STATUS, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(FILES_PATH_PROVIDER_STATUS), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(FILES_PATH_PROVIDER_STATUS, 'loader', true)(), [
	]);
	...
	React.useEffect(() => {
		if (!unmount) {
			actionApiListGet(FILES_PATH_PROVIDER_STATUS, {
				page,
				limit,
				query,
				select,
	...
	React.useEffect(() => () => actionApiListClear(FILES_PATH_PROVIDER_STATUS)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={FILES_PATH_PROVIDER_STATUS_CREATE} />}
			storePath={storePath}
		...
		<TableStatus
			...
			{utilsCheckArr(data)
				&& data.map((item) => <TableStatusItem
					...
					key={item.id}
					id={item.id}
					to={`/files/provider/statuses/${item.id}`}
					name={item.name}
