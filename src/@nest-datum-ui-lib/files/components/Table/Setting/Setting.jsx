...
let Setting = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SETTING, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SETTING, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SETTING, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SETTING, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SETTING, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', FILES_PATH_SETTING ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(FILES_PATH_SETTING, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(FILES_PATH_SETTING, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(FILES_PATH_SETTING, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(FILES_PATH_SETTING, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(FILES_PATH_SETTING, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(FILES_PATH_SETTING, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(FILES_PATH_SETTING, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(FILES_PATH_SETTING, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(FILES_PATH_SETTING), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(FILES_PATH_SETTING, 'loader', true)(), [
	]);
	...
	React.useEffect(() => {
		if (!unmount) {
			actionApiListGet(FILES_PATH_SETTING, {
				page,
				limit,
				query,
				select,
	...
	React.useEffect(() => () => actionApiListClear(FILES_PATH_SETTING)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={FILES_PATH_SETTING_CREATE} />}
			storePath={storePath}
