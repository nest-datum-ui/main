...
let Options = ({
	...
	const onDelete = React.useCallback((systemId) => (e) => {
		actionDialogOpen('filesSystemReferenceDrop', {
			systemId,
			optionId: entityId,
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										<IconButton onClick={onDelete(item['systemSystemOptions'][0]['id'])}>
...
Options.defaultProps = {
	...
	storeName: 'filesSystemOptionList',
