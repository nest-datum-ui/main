...
import { 
	...
	FILES_PATH_SYSTEM_OPTION_CREATE, 
...
let Option = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SYSTEM_OPTION, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SYSTEM_OPTION, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SYSTEM_OPTION, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SYSTEM_OPTION, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', FILES_PATH_SYSTEM_OPTION, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', FILES_PATH_SYSTEM_OPTION ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(FILES_PATH_SYSTEM_OPTION, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(FILES_PATH_SYSTEM_OPTION, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(FILES_PATH_SYSTEM_OPTION, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(FILES_PATH_SYSTEM_OPTION, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(FILES_PATH_SYSTEM_OPTION, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(FILES_PATH_SYSTEM_OPTION, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(FILES_PATH_SYSTEM_OPTION, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(FILES_PATH_SYSTEM_OPTION, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(FILES_PATH_SYSTEM_OPTION), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(FILES_PATH_SYSTEM_OPTION, 'loader', true)(), [
	]);
	...
	React.useEffect(() => () => actionApiListClear(FILES_PATH_SYSTEM_OPTION)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={FILES_PATH_SYSTEM_OPTION_CREATE} />}
			storePath={storePath}
		...
		<TableOption
			...
			{utilsCheckArr(data)
				&& data.map((item) => <TableOptionItem
					...
					key={item.id}
					id={item.id}
					to={`/files/system/options/${item.id}`}
					name={item.name}
