...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FILES_PATH_PROVIDER, 'loader', true)();
	...
	!utilsCheckStrId(providerStatusId)
		&& (errors['providerStatusId'] = 'The value is in the wrong format.');
	(utilsCheckExists(isNotDelete) && !utilsCheckBool(isNotDelete))
		&& (errors['isNotDelete'] = 'The value is in the wrong format.');

	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FILES_PATH_PROVIDER, 'errors', errors)();
		await actionApiFormProp(FILES_PATH_PROVIDER, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FILES_PATH_PROVIDER, entityId)();
		actionApiFormCreateOptions(FILES_PATH_PROVIDER_OPTION, { path: `${FILES_PATH_PROVIDER}/${entityId}/options` })();
	}
	else {
		actionApiFormCreate(FILES_PATH_PROVIDER)();
		actionApiFormCreateOptions(FILES_PATH_PROVIDER_OPTION, { path: `${FILES_PATH_PROVIDER}/${entityId}/options` })();
	}
};
