...
const onCreate = async ({
	...
	try {
		...
		const { providerId } = Store().getState()['api'].form[entityId];
		const errors = {};

		apiPath = `provider/${providerId}/options/${entityId}`;

		if (!validateStr(providerId, true)) {
			errors['providerId'] = 'The value is in the wrong format.';
		}
		else if (!validateNotEmpty(providerId)) {
			errors['providerId'] = 'Cannot be empty.';
		}
		...
		actionApiFormCreate({
			...
			path: `provider/${providerId}/options/${entityId}`,
			...
			await actionDialogClose('filesProviderReferenceItem')();
