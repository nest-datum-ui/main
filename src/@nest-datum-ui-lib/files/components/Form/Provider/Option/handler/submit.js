...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FILES_PATH_PROVIDER_OPTION, 'loader', true)();
	...
	const {
		...
		.getState()
		.api || {})
		.form || {})[FILES_PATH_PROVIDER_OPTION] || {};
	const errors = {};
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FILES_PATH_PROVIDER_OPTION, 'errors', errors)();
		await actionApiFormProp(FILES_PATH_PROVIDER_OPTION, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FILES_PATH_PROVIDER_OPTION, entityId)();
	}
	else {
		actionApiFormCreate(FILES_PATH_PROVIDER_OPTION)();
	}
};
