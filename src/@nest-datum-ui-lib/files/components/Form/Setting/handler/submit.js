...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FILES_PATH_SETTING, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FILES_PATH_SETTING, 'errors', errors)();
		await actionApiFormProp(FILES_PATH_SETTING, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FILES_PATH_SETTING, entityId)();
	}
	else {
		actionApiFormCreate(FILES_PATH_SETTING)();
	}
};
