...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FILES_PATH_SYSTEM, 'loader', true)();
	...
	!utilsCheckStrId(providerId)
		&& (errors['providerId'] = 'The value is in the wrong format.');
	!utilsCheckStrId(systemStatusId)
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FILES_PATH_SYSTEM, 'errors', errors)();
		await actionApiFormProp(FILES_PATH_SYSTEM, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FILES_PATH_SYSTEM, entityId)();
		actionApiFormCreateOptions(FILES_PATH_SYSTEM_OPTION, { path: `${FILES_PATH_SYSTEM}/${entityId}/options` })();
	}
	else {
		actionApiFormCreate(FILES_PATH_SYSTEM)();
		actionApiFormCreateOptions(FILES_PATH_SYSTEM_OPTION, { path: `${FILES_PATH_SYSTEM}/${entityId}/options` })();
	}
};
