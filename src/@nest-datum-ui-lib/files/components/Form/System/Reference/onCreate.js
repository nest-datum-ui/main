...
const onCreate = async ({
	...
	try {
		...
		const { systemId } = Store().getState()['api'].form[entityId];
		const errors = {};

		apiPath = `system/${systemId}/options/${entityId}`;

		if (!validateStr(systemId, true)) {
			errors['systemId'] = 'The value is in the wrong format.';
		}
		else if (!validateNotEmpty(systemId)) {
			errors['systemId'] = 'Cannot be empty.';
		}
		...
		actionApiFormCreate({
			...
			path: `system/${systemId}/options/${entityId}`,
			...
			await actionDialogClose('filesSystemReferenceItem')();
