...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FILES_PATH_SYSTEM_STATUS, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FILES_PATH_SYSTEM_STATUS, 'errors', errors)();
		await actionApiFormProp(FILES_PATH_SYSTEM_STATUS, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FILES_PATH_SYSTEM_STATUS, entityId)();
	}
	else {
		actionApiFormCreate(FILES_PATH_SYSTEM_STATUS)();
	}
};
