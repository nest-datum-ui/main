...
import { FILES_PATH_SYSTEM_OPTION } from '@nest-datum-ui-lib/files/consts/path.js';
...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(FILES_PATH_SYSTEM_OPTION, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(FILES_PATH_SYSTEM_OPTION, 'errors', errors)();
		await actionApiFormProp(FILES_PATH_SYSTEM_OPTION, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(FILES_PATH_SYSTEM_OPTION, entityId)();
	}
	else {
		actionApiFormCreate(FILES_PATH_SYSTEM_OPTION)();
	}
};
