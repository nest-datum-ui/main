...
import MailFormFilterStatusLetter from '@nest-datum-ui-lib/mail/components/Form/Filter/Status/Letter';
...
let Form = () => {
	...
	const storePath = React.useMemo(() => [ 'api', 'list', MAIL_PATH_LETTER ], [
	]);
	...
	const onSortId = React.useCallback((value) => actionApiListSort(MAIL_PATH_LETTER, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(MAIL_PATH_LETTER, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(MAIL_PATH_LETTER, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(MAIL_PATH_LETTER, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(MAIL_PATH_LETTER, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(MAIL_PATH_LETTER, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(MAIL_PATH_LETTER), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(MAIL_PATH_LETTER, 'loader', true)(), [
	]);
	...
	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={MAIL_PATH_LETTER_CREATE} />}
			storePath={storePath}
			...
			<MailFormFilterStatusLetter onInput={onLoader} />
		</FormFilter>
		{(!displayLoader)
			&& <React.Fragment>
				<TablePagination
					...
					{data.map((item) => <Item
						...
						letterStatusId={item.letterStatusId}
						userId={item.userId}
