...
import { 
	...
	MAIL_PATH_TEMPLATE_OPTION_CREATE, 
...
let Option = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_OPTION, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_OPTION, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_OPTION, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_OPTION, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_OPTION, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', MAIL_PATH_TEMPLATE_OPTION ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(MAIL_PATH_TEMPLATE_OPTION, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(MAIL_PATH_TEMPLATE_OPTION, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(MAIL_PATH_TEMPLATE_OPTION, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(MAIL_PATH_TEMPLATE_OPTION, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(MAIL_PATH_TEMPLATE_OPTION, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(MAIL_PATH_TEMPLATE_OPTION, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(MAIL_PATH_TEMPLATE_OPTION, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(MAIL_PATH_TEMPLATE_OPTION, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(MAIL_PATH_TEMPLATE_OPTION), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(MAIL_PATH_TEMPLATE_OPTION, 'loader', true)(), [
	]);
	...
	React.useEffect(() => () => actionApiListClear(MAIL_PATH_TEMPLATE_OPTION)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={MAIL_PATH_TEMPLATE_OPTION_CREATE} />}
			storePath={storePath}
		...
		<TableOption
			...
			{utilsCheckArr(data)
				&& data.map((item) => <TableOptionItem
					...
					key={item.id}
					id={item.id}
					to={`/mail/template/options/${item.id}`}
					name={item.name}
