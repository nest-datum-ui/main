...
let Options = ({
	...
	const onDelete = React.useCallback((templateId) => (e) => {
		actionDialogOpen('mailTemplateReferenceDrop', {
			templateId,
			optionId: entityId,
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									<TableCell sx={{ minWidth: '18%' }}>
										<Typography 
											component={Link}
											to={`/${serviceKey}/template/${item.id}`}
									...
									<TableCell sx={{ width: '1%' }}>
										<IconButton onClick={onDelete(item['templateTemplateOptions'][0]['id'])}>
...
Options.defaultProps = {
	...
	storeName: 'mailTemplateOptionList',
