...
import { 
	MAIL_PATH_TEMPLATE_STATUS,
	MAIL_PATH_TEMPLATE_STATUS_CREATE, 
...
let Status = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_STATUS, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_STATUS, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_STATUS, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_STATUS, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_STATUS, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', MAIL_PATH_TEMPLATE_STATUS ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(MAIL_PATH_TEMPLATE_STATUS, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(MAIL_PATH_TEMPLATE_STATUS, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(MAIL_PATH_TEMPLATE_STATUS, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(MAIL_PATH_TEMPLATE_STATUS, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(MAIL_PATH_TEMPLATE_STATUS, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(MAIL_PATH_TEMPLATE_STATUS, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(MAIL_PATH_TEMPLATE_STATUS, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(MAIL_PATH_TEMPLATE_STATUS, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(MAIL_PATH_TEMPLATE_STATUS), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(MAIL_PATH_TEMPLATE_STATUS, 'loader', true)(), [
	]);
	...
	React.useEffect(() => {
		if (!unmount) {
			actionApiListGet(MAIL_PATH_TEMPLATE_STATUS, {
				page,
				limit,
				query,
				select,
	...
	React.useEffect(() => () => actionApiListClear(MAIL_PATH_TEMPLATE_STATUS)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={MAIL_PATH_TEMPLATE_STATUS_CREATE} />}
			storePath={storePath}
		...
		<TableStatus
			...
			{utilsCheckArr(data)
				&& data.map((item) => <TableStatusItem
					...
					key={item.id}
					id={item.id}
					to={`/mail/template/statuses/${item.id}`}
					name={item.name}
