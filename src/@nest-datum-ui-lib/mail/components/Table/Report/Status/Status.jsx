...
import { 
	MAIL_PATH_REPORT_STATUS,
	MAIL_PATH_REPORT_STATUS_CREATE, 
...
let Status = () => {
	...
	const storePath = React.useMemo(() => [ 'api', 'list', MAIL_PATH_REPORT_STATUS ], [
	]);
	...
	const onSortId = React.useCallback((value) => actionApiListSort(MAIL_PATH_REPORT_STATUS, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(MAIL_PATH_REPORT_STATUS, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(MAIL_PATH_REPORT_STATUS, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(MAIL_PATH_REPORT_STATUS, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(MAIL_PATH_REPORT_STATUS, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(MAIL_PATH_REPORT_STATUS, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(MAIL_PATH_REPORT_STATUS), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(MAIL_PATH_REPORT_STATUS, 'loader', true)(), [
	]);
	...
	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={MAIL_PATH_REPORT_STATUS_CREATE} />}
			storePath={storePath}
		...
		<TableStatus
			...
			{utilsCheckArr(data)
				&& data.map((item) => <TableStatusItem
					...
					key={item.id}
					id={item.id}
					to={`/mail/report/statuses/${item.id}`}
					name={item.name}
