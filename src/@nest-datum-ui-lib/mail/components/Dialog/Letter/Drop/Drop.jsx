...
let Drop = () => {
	const entityId = useSelector(selectorMainExtract([ 'dialog', MAIL_PATH_LETTER, 'entityId' ]));
	const formLoader = useSelector(selectorMainExtract([ 'api', 'form', MAIL_PATH_LETTER, 'loader' ]));
	const listLoader = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_LETTER, 'loader' ]));
	const onDrop = React.useCallback((e) => actionApiFormDrop(MAIL_PATH_LETTER, entityId)(), [
		entityId,
	]);
	...
	return <React.Fragment>
		<Dialog 
			...
			<DialogContentText>
				Are you sure you want to delete the current letter? This operation is irreversible and may compromise data integrity.
			</DialogContentText>
		</Dialog>
	</React.Fragment>;
};
