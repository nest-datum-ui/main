...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(MAIL_PATH_LETTER_OPTION, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(MAIL_PATH_LETTER_OPTION, 'errors', errors)();
		await actionApiFormProp(MAIL_PATH_LETTER_OPTION, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(MAIL_PATH_LETTER_OPTION, entityId)();
	}
	else {
		actionApiFormCreate(MAIL_PATH_LETTER_OPTION)();
	}
};
