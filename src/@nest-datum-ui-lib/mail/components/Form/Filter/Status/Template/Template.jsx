...
import MailSelectTemplateStatusMultiple from '@nest-datum-ui-lib/mail/components/Select/Template/Status/Multiple';
...
let Template = ({
	...
	const dataLength = useSelector(selectorMainExtract([ 'api', 'list', MAIL_PATH_TEMPLATE_STATUS, 'data', 'length' ]));
	const value = (dataLength > 0)
		? utilsUrlItemFilterGetId(search, MAIL_PATH_TEMPLATE_STATUS, 'templateStatusId')
		: [];
	const onChangeMemo = React.useCallback((e) => {
		actionApiListCreateFilterUrl(e, utilsUrlItemFilterId('templateStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
	...
	const onClear = React.useCallback((e) => {
		actionApiListClearFilterUrl(utilsUrlItemFilterId('templateStatusId'));
		onChange(e);
		onInput(e);
	}, [
		onChange,
	...
	return <React.Fragment>
		<Grid
			...
			<Grid
				item
				xs={true}>
				<MailSelectTemplateStatusMultiple 
					label="Status"
					name={id}
