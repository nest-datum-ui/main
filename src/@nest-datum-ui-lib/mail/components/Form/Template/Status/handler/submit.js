...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(MAIL_PATH_TEMPLATE_STATUS, 'loader', true)();
	...
	const {
		...
		.getState()
		.api || {})
		.form || {})[MAIL_PATH_TEMPLATE_STATUS] || {};
	const errors = {};
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(MAIL_PATH_TEMPLATE_STATUS, 'errors', errors)();
		await actionApiFormProp(MAIL_PATH_TEMPLATE_STATUS, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(MAIL_PATH_TEMPLATE_STATUS, entityId)();
	}
	else {
		actionApiFormCreate(MAIL_PATH_TEMPLATE_STATUS)();
	}
};
