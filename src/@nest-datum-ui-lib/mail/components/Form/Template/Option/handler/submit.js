...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(MAIL_PATH_TEMPLATE_OPTION, 'loader', true)();
	...
	const {
		...
		.getState()
		.api || {})
		.form || {})[MAIL_PATH_TEMPLATE_OPTION] || {};
	const errors = {};
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(MAIL_PATH_TEMPLATE_OPTION, 'errors', errors)();
		await actionApiFormProp(MAIL_PATH_TEMPLATE_OPTION, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(MAIL_PATH_TEMPLATE_OPTION, entityId)();
	}
	else {
		actionApiFormCreate(MAIL_PATH_TEMPLATE_OPTION)();
	}
};
