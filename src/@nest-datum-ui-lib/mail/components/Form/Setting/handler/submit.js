...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(MAIL_PATH_SETTING, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(MAIL_PATH_SETTING, 'errors', errors)();
		await actionApiFormProp(MAIL_PATH_SETTING, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(MAIL_PATH_SETTING, entityId)();
	}
	else {
		actionApiFormCreate(MAIL_PATH_SETTING)();
	}
};
