...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(MAIL_PATH_REPORT_STATUS, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(MAIL_PATH_REPORT_STATUS, 'errors', errors)();
		await actionApiFormProp(MAIL_PATH_REPORT_STATUS, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(MAIL_PATH_REPORT_STATUS, entityId)();
	}
	else {
		actionApiFormCreate(MAIL_PATH_REPORT_STATUS)();
	}
};
