...
let Status = ({
	...
	const name = useSelector(selectorMainExtract([ 'api', 'form', `${MAIL_PATH_LETTER_STATUS}/${children}`, 'name' ]));
	...
	React.useEffect(() => () => {
		actionApiFormClear(`${MAIL_PATH_LETTER_STATUS}/${children}`)();
		actionApiFormDropLoop(`${MAIL_PATH_LETTER_STATUS}/${children}`);
	}, [
		children,
	]);
