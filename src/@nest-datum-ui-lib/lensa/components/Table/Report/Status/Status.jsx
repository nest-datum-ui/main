...
import { 
	CV_PATH_REPORT_STATUS,
	CV_PATH_REPORT_STATUS_CREATE, 
...
let Status = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', CV_PATH_REPORT_STATUS, 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', CV_PATH_REPORT_STATUS, 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', CV_PATH_REPORT_STATUS, 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', CV_PATH_REPORT_STATUS, 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', CV_PATH_REPORT_STATUS, 'data' ]));
	const storePath = React.useMemo(() => [ 'api', 'list', CV_PATH_REPORT_STATUS ], [
	]);
	const displayLoader = !utilsCheckArr(data) || unmount || loader;
	const onChangePage = React.useCallback((e, newPage) => actionApiListPage(CV_PATH_REPORT_STATUS, newPage), [
	]);
	const onLimit = React.useCallback((e) => actionApiListLimit(CV_PATH_REPORT_STATUS, e), [
	]);
	const onSortId = React.useCallback((value) => actionApiListSort(CV_PATH_REPORT_STATUS, 'id', value), [
	]);
	const onSortCreatedAt = React.useCallback((value) => actionApiListSort(CV_PATH_REPORT_STATUS, 'createdAt', value), [
	]);
	const onDrop = React.useCallback((id) => (e) => actionApiListDrop(CV_PATH_REPORT_STATUS, id), [
	]);
	const onRestore = React.useCallback((id) => (e) => actionApiListRestore(CV_PATH_REPORT_STATUS, id), [
	]);
	const onCheck = React.useCallback((id) => actionApiListCheck(CV_PATH_REPORT_STATUS, id), [
	]);
	const onBulk = React.useCallback((e) => actionApiListBulk(CV_PATH_REPORT_STATUS, e), [
	]);
	const onBulkDrop = React.useCallback(() => actionApiListBulkDrop(CV_PATH_REPORT_STATUS), [
	]);
	const onLoader = React.useCallback(() => actionApiListProp(CV_PATH_REPORT_STATUS, 'loader', true)(), [
	]);
	...
	React.useEffect(() => {
		if (!unmount) {
			actionApiListGet(CV_PATH_REPORT_STATUS, {
				page,
				limit,
				query,
				select,
	...
	React.useEffect(() => () => actionApiListClear(CV_PATH_REPORT_STATUS)(), [
	]);

	return <React.Fragment>
		<FormFilter 
			bulkDeletion
			toolbarComponent={<ButtonCreate to={CV_PATH_REPORT_STATUS_CREATE} />}
			storePath={storePath}
