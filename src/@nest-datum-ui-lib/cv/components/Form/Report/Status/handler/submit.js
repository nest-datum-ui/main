...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(CV_PATH_REPORT_STATUS, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(CV_PATH_REPORT_STATUS, 'errors', errors)();
		await actionApiFormProp(CV_PATH_REPORT_STATUS, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(CV_PATH_REPORT_STATUS, entityId)();
	}
	else {
		actionApiFormCreate(CV_PATH_REPORT_STATUS)();
	}
};
