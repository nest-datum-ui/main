...
import { CV_PATH_SETTING } from '@nest-datum-ui-lib/cv/consts/path.js';
...
let Setting = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'form', CV_PATH_SETTING, 'loader' ]));
	const formLength = useSelector(selectorMainExtract([ 'api', 'form', CV_PATH_SETTING ], (formObj) => Object.keys(formObj || {}).length));
	const isNotDelete = useSelector(selectorMainExtract([ 'api', 'form', CV_PATH_SETTING, 'isNotDelete' ]));
	const isDeleted = useSelector(selectorMainExtract([ 'api', 'form', CV_PATH_SETTING, 'isDeleted' ]));
	const onSubmit = React.useCallback((e) => handlerSubmit(e, entityId), [
		entityId,
	]);
	const onDrop = React.useCallback((e) => actionDialogOpen(CV_PATH_SETTING, { entityId })(), [
		entityId,
	]);

	React.useEffect(() => {
		if (!unmount) {
			actionApiFormGet(CV_PATH_SETTING, entityId)();
		}
	}, [
		unmount,
		entityId,
	...
	React.useEffect(() => () => actionApiFormClear(CV_PATH_SETTING)(), [
	]);
	...
	return <React.Fragment>
		<Form 
			...
			<InputId storeFormName={CV_PATH_SETTING} />
			<InputName storeFormName={CV_PATH_SETTING} />
			<InputDescription storeFormName={CV_PATH_SETTING} />
