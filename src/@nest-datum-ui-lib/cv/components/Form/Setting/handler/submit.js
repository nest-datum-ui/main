...
const submit = async (e, entityId) => {
	...
	await actionApiFormProp(CV_PATH_SETTING, 'loader', true)();
	...
	if (Object.keys(errors).length > 0) {
		await actionApiFormProp(CV_PATH_SETTING, 'errors', errors)();
		await actionApiFormProp(CV_PATH_SETTING, 'loader', false)();
	}
	else if (utilsCheckEntityExists(entityId)) {
		actionApiFormUpdate(CV_PATH_SETTING, entityId)();
	}
	else {
		actionApiFormCreate(CV_PATH_SETTING)();
	}
};
