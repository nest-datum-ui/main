...
import MenuSettingContext from '@nest-datum-ui/components/Menu/Setting/Context';
...
let Setting = ({
	...
	const onMenu = React.useCallback((itemId) => (e) => {
		actionMenuOpen(`menu-setting-context-${itemId}`, e.target)();
	}, [
	]);
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										...
										<MenuSettingContext 
											id={`menu-setting-context-${item.id}`}
											entityId={item.id}
