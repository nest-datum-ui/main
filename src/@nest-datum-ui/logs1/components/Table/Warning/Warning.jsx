...
import MenuWarningContext from '../../Menu/Warning/Context';
...
let Warning = () => {
	...
	const loader = useSelector(selectorMainExtract([ 'api', 'list', 'warningList', 'loader' ]));
	const total = useSelector(selectorMainExtract([ 'api', 'list', 'warningList', 'total' ])) ?? 0;
	const page = useSelector(selectorMainExtract([ 'api', 'list', 'warningList', 'page' ])) ?? 1;
	const limit = useSelector(selectorMainExtract([ 'api', 'list', 'warningList', 'limit' ])) ?? 20;
	const data = useSelector(selectorMainExtract([ 'api', 'list', 'warningList', 'data' ]));
	...
	const onChangePage = React.useCallback((e, newPage) => {
		actionApiListProp('warningList', 'loader', true)();
		actionApiListProp('warningList', 'page', newPage)();
	}, [
	]);
	const onLimit = React.useCallback((e) => {
		actionApiListProp('warningList', 'loader', true)();
		actionApiListProp('warningList', 'limit', e.target.value)();
	}, [
	]);
	...
	const onMenu = React.useCallback((itemId) => (e) => {
		actionMenuOpen(`logs-menu-warning-context-${itemId}`, e.target)();
	}, [
	]);
	...
	React.useEffect(() => () => {
		actionApiListClear('warningList')();
	}, []);
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										...
										<MenuWarningContext 
											id={`logs-menu-warning-context-${item.id}`}
											entityId={item.id} />
									</TableCell>
								</TableRow>;
							})}
