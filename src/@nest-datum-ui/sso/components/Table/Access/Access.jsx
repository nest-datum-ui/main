...
import MenuAccessContext from '@nest-datum-ui/sso/components/Menu/Access/Context';
...
let Access = ({
	...
	const onMenu = React.useCallback((itemId) => (e) => {
		actionMenuOpen(`sso-menu-access-context-${itemId}`, e.target)();
	}, [
	]);
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										...
										<MenuAccessContext 
											id={`sso-menu-access-context-${item.id}`}
											entityId={item.id}
