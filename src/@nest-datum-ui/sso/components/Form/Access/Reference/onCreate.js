...
const onCreate = async ({
	...
	try {
		...
		const { accessId } = Store().getState()['api'].form[entityId];
		const errors = {};

		apiPath = `access/${accessId}/options/${entityId}`;

		if (!validateStr(accessId, true)) {
			errors['accessId'] = 'The value is in the wrong format.';
		}
		else if (!validateNotEmpty(accessId)) {
			errors['accessId'] = 'Cannot be empty.';
		}
		...
		actionApiFormCreate({
			...
			path: `access/${accessId}/options/${entityId}`,
			...
			await actionDialogClose('ssoAccessReferenceItem')();
