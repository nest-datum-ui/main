...
import MenuTypeContext from '@nest-datum-ui/data-type/components/Menu/Type/Context';
...
let Type = ({
	...
	const onMenu = React.useCallback((itemId) => (e) => {
		actionMenuOpen(`dataType-menu-type-context-${itemId}`, e.target)();
	}, [
	]);
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										...
										<MenuTypeContext 
											id={`dataType-menu-type-context-${item.id}`}
											entityId={item.id}
