...
let Options = ({
	...
	const onDelete = React.useCallback((letterId) => (e) => {
		actionDialogOpen('mailLetterReferenceDrop', {
			letterId,
			optionId: entityId,
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										<IconButton onClick={onDelete(item['letterLetterOptions'][0]['id'])}>
...
Options.defaultProps = {
	...
	storeName: 'mailLetterOptionList',
