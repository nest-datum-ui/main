...
import MenuReportContext from '@nest-datum-ui/mail/components/Menu/Report/Context';
...
let Report = ({
	...
	const onMenu = React.useCallback((itemId) => (e) => {
		actionMenuOpen(`mail-menu-report-context-${itemId}`, e.target)();
	}, [
	]);
							...
							{data.map((item, index) => {
								return <TableRow key={item.id}>
									...
									<TableCell sx={{ width: '1%' }}>
										...
										<MenuReportContext 
											id={`mail-menu-report-context-${item.id}`}
											entityId={item.id}
