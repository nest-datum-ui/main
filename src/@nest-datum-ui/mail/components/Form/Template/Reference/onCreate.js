...
const onCreate = async ({
	...
	try {
		...
		const { templateId } = Store().getState()['api'].form[entityId];
		const errors = {};

		apiPath = `template/${templateId}/options/${entityId}`;

		if (!validateStr(templateId, true)) {
			errors['templateId'] = 'The value is in the wrong format.';
		}
		else if (!validateNotEmpty(templateId)) {
			errors['templateId'] = 'Cannot be empty.';
		}
		...
		actionApiFormCreate({
			...
			path: `template/${templateId}/options/${entityId}`,
			...
			await actionDialogClose('mailTemplateReferenceItem')();
