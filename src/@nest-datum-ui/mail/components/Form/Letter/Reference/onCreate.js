...
const onCreate = async ({
	...
	try {
		...
		const { letterId } = Store().getState()['api'].form[entityId];
		const errors = {};

		apiPath = `letter/${letterId}/options/${entityId}`;

		if (!validateStr(letterId, true)) {
			errors['letterId'] = 'The value is in the wrong format.';
		}
		else if (!validateNotEmpty(letterId)) {
			errors['letterId'] = 'Cannot be empty.';
		}
		...
		actionApiFormCreate({
			...
			path: `letter/${letterId}/options/${entityId}`,
			...
			await actionDialogClose('mailLetterReferenceItem')();
